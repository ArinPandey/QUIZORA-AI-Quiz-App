...
import { Zap, Plus, X, Sliders, Brain } from 'lucide-react';
import toast from 'react-hot-toast';

const InstantQuiz = () => {
  const navigate = useNavigate();  //For redirecting to the quiz view after generation
  const { token } = useSelector((state) => state.auth);
  const [topic, setTopic] = useState("");  //Stores the manual topic typed by the user
  const [selectedTags, setSelectedTags] = useState([]); //Preset tags for quick selection
  const [customTags, setCustomTags] = useState([]);  //Stores tags that the user creates themselves
  const [numQuestions, setNumQuestions] = useState(5);  //Range slider state (starts at 5)
  const [difficulty, setDifficulty] = useState("Medium");  //Difficulty selection state

  //Logic to add a tag when the user clicks the '+' button
  const handleAddTag = () => {
    // Calculate total tags (preset + custom)
  ...
  //Calculate total tags for UI display
  const totalTags = selectedTags.length + customTags.length;
  const isMaxTags = totalTags >= 3;
      ...
      <div className="bg-white/50 backdrop-blur-md p-8 rounded-3xl shadow-2xl w-full max-w-lg border border-white/20">
        ...
        <div className="space-y-4 mb-8">
          <div className="flex justify-between items-center">
            ...
            <span className={`text-xs font-bold ${isMaxTags ? 'text-red-500' : 'text-gray-400'}`}>
              {totalTags}/3 tags
            </span>
          </div>
          ...
          <div className="flex space-x-2">
            ...
            />
            {/* Disable button and change style when max tags reached */}
            <button 
...
//         {/* Topic / Custom Tag Input Section */}
//         <div className="space-y-4 mb-8">
...
//           {/* Active Tags Container (Displays both preset and custom tags) */}
//           <div className="flex flex-wrap gap-2">
//             {[...selectedTags, ...customTags].map((tag) => (
//               <span key={tag} className="flex items-center space-x-1 bg-orange-100 text-orange-700 px-3 py-1.5 rounded-full text-sm font-medium border border-orange-200">
//                 <span>{tag}</span>
...
//         {/* Range Slider for Question Count (5 to 10) */}
//         <div className="mb-8">
//           <div className="flex justify-between items-center mb-2">
//             <label className="text-sm font-semibold text-gray-500 uppercase">Number of Questions</label>
//             <span className="text-orange-600 font-bold text-lg">{numQuestions}</span>
