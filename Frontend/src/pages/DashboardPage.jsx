...
// 1. Import the AlertCircle icon for displaying errors
import { UploadCloud, File, X, AlertCircle } from 'lucide-react'; 
import { generateQuiz } from '../operations/quizAPI';

const DashboardPage = () => {
    // 2. Get the new 'error' state from the quiz slice in Redux
    const { loading: quizLoading, error: quizError } = useSelector((state) => state.quiz); 
    ...
    // 3. The handleUpload function is now simpler.
    // It just dispatches the action. The error handling logic is inside the Redux thunk.
    const handleUpload = () => {
        if (!selectedFile) {
            alert("Please select a file first.");
            return;
        }
        dispatch(generateQuiz(selectedFile, navigate));
    };
        ...
        <div className="min-h-[calc(100vh-128px)] p-4 md:p-8 mt-[-25px] bg-gradient-to-br from-rose-300 via-pink-400 to-purple-400">
            <div className="max-w-5xl mx-auto">
                ...
                <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-gray-200">
                    ...
                    {/* 4. This is the new block to display the error message */}
                    {quizError && (
                        <div className="mt-4 p-4 bg-red-50 text-red-700 border border-red-200 rounded-lg flex items-center space-x-3 animate-fade-in">
                            <AlertCircle />
                            <span>{quizError}</span>
                        </div>
                    )}
